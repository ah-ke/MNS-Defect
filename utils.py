...
def plot_segmentation_images(
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        # savename = os.path.normpath(image_path).split('\\')
        ...
        image_savename = os.path.join(savefolder, savename + "_image.png")
        plt.imshow(image.transpose(1, 2, 0))
        plt.axis('off')
        plt.savefig(image_savename)
        plt.close()

        # segmentation热力图
        segmentation_savename = os.path.join(savefolder, savename + "_heatmap.png")
        segmentation_heatmap = (segmentation * 255).astype(np.uint8)
        segmentation_heatmap = cv2.applyColorMap(segmentation_heatmap, cv2.COLORMAP_JET)  # 生成heat map
        segmentation_heatmap = cv2.addWeighted(image.transpose(1, 2, 0), 1, segmentation_heatmap, 0.3, 0)
        segmentation_heatmap = segmentation_heatmap[:, :, ::-1] #注意cv2（BGR）和matplotlib(RGB)通道是相反的
        plt.imshow(segmentation_heatmap)
        plt.axis('off')
        plt.savefig(segmentation_savename, dpi=100)
        plt.close()


        #image叠加mask
        mask_savename = os.path.join(savefolder, savename + "_image_mask.png")
        mask_bool = mask[0] > 0
        image_mask = image.transpose(1, 2, 0).astype(np.uint8)
        for i in range(mask_bool.shape[0]):
            for j in range(mask_bool.shape[1]):
                if mask_bool[i][j]:
                    image_mask[i][j][0] = 255
                    image_mask[i][j][1] = 0
                    image_mask[i][j][2] = 0
        plt.imshow(image_mask)
        plt.axis('off')
        plt.savefig(mask_savename)
        plt.close()
